...
<script type="text/javascript">
	...
	function add(){
		 ...
		 diag.Drag=true;
		 diag.Title ="新增";
		 diag.URL = '<%=basePath%>hudongCategory/toAdd.do';
		 diag.Width = 223;
		 diag.Height = 125;
	...
	function edit(tagID){
		 ...
		 diag.Drag=true;
		 diag.Title ="编辑";
		 diag.URL = '<%=basePath%>hudongCategory/toEdit.do?tagID='+tagID;
		 diag.Width = 223;
		 diag.Height = 125;
		 diag.CancelEvent = function(){ //关闭事件
			 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
	...
	//删除
		function del(tagID){
			bootbox.confirm("确定要删除该条数据吗?", function(result) {
				if(result) {
					top.jzts();
					var url = '<%=basePath%>hudongCategory/delete.do?tagID='+tagID;
			...
			bootbox.confirm('确定执行批量删除操作吗？',function(result){
				if(result){
					...
					if(str==''){
						...
						$.ajax({
							type:'post',
							url:'<%=basePath%>hudongCategory/deleteAll.do',
							dataTyoe:'json',
							cache: false,
							data:{
								"USER_ids":str
...
<div class="row-fluid">
	...
	<!-- 检索  -->
	<form action="<%=basePath%>/hudongCategory/getlistPage.do" method="post" name="userForm" id="userForm">
	<table>
		<tr>
			<td>
			...
			<c:when test="${not empty list}">
				...
				<tr>
					<td class='center' style="width: 30px;">
				 		<label><input type='checkbox' name='ids' value="${var.hudong_category_id }"/><span class="lbl"></span></label>
					</td>
