...
<div id="page-content" class="clearfix">
  <div class="row-fluid">
	<div class="row-fluid">
			<!-- 检索开始  -->
			<form action="orderChangtu/orderChangtuListPage.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<!-- 开始循环 -->	
				<c:choose>
					<c:when test="${not empty orderChangtuList}">
						<c:if test="${QX.cha == 1 }">
							<c:forEach items="${orderChangtuList}" var="var" varStatus="vs">
								<tr>
									<td class='center' style="width: 30px;">
										<label><input type='checkbox' name='ids' value="${var.order_changtu_id}" /><span class="lbl"></span></label>
									</td>
									<td class='center' style="width: 30px;">${vs.index+1}</td>
											<td class="center">${var.userName}</td>
											<td class="center">${var.phone}</td>
											<td class="center">${var.changtu_mileage}</td>
											<td class="center">${var.changtu_departurePlace}</td>
											<td class="center">${var.changtu_destination}</td>
											<td class="center">${var.changtu_radeAmount}</td>
											...
											<td class="center">${var.order_changtu_Time}</td>
											<td style="width: 68px;">
												<a class='btn btn-mini btn-info' title="编辑" onclick="edit('${var.order_changtu_id }')" ><i class='icon-edit'></i></a>
												<a class='btn btn-mini btn-danger' title="删除"  onclick="del('${var.order_changtu_id }')"><i class='icon-trash'></i></a>
											</td>
								</tr>
