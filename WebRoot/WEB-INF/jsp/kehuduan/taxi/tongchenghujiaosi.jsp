...
<div id="header" style="background-color: #068dff;">
  ...
  <a href="api/h5KeHu/index.do" class="icon1" style="margin-top:14px;"><i class="icon-angle-left "></i></a>
</div>
...
<div class="dacheOrder" style="margin-top:60px;">
    <div class="hujiaoCir">
        ...
        <p>司机已接驾乘客...</p>
        <p>
        	<c:if test="${pds.order_tongcheng_status=='4' }">大约${pds.about_Amount }元</c:if>
        	<c:if test="${pds.order_tongcheng_status=='2' }">大约${pds.radeAmount }元</c:if>
        </p>
    </div>
    <span style="color: red; font-size: 13px;">小提示：如您已到达目的地，请点击下方去付款按钮前往付款，如退出当前页，则在个人中心处"打车订单"点击"未付款"状态订单并支付。</span>
     <div class="yanzheng" style="background-color: #C7C6CC;width:100%;">
     	<a href="<%=basePath %>api/h5KeHu/topaytcdc.do?order_tongcheng_id=${pds.order_tongcheng_id}&radeAmount=${pds.radeAmount }" style="width:100%;margin:10px auto;border-radius: 10px;">去付款</a>
     </div> 
</div>
...
<script>
	function qufukuai(id){
		var id =$("#tid").val();
		//var respCode ="${respCode}";
		//if (respCode =="01") {
		layer.alert("您确定要去付款吗！",{
